...
- *(Optional)* An [ElevenLabs](https://elevenlabs.io) API key (for reusable ambient loops and sound effects)
...
Stories can include up to two longer ambient soundscape loops (forest sounds, rain, etc.) reused across pages, plus up to two total event sound effects (doors creaking, footsteps, etc.) generated by [ElevenLabs](https://elevenlabs.io). Sound effects are optional — add an ElevenLabs API key in Settings or during setup to enable them. You can generate audio for existing stories from the book debug page.
</details>
